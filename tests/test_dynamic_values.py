...
def test_date_placeholder():
    ...
    with patch.object(manager, '_read_prompts', return_value={
        "DATE_TEST": "Today is {{date:format=%Y-%m-%d}}."
    }):
...
def test_number_placeholder():
    ...
    with patch.object(manager, '_read_prompts', return_value={
        "NUMBER_TEST": "Your number is {{number:min=1,max=10}}."
    }):
...
def test_choice_placeholder():
    ...
    with patch.object(manager, '_read_prompts', return_value={
        "CHOICE_TEST": f"I recommend {{{{choice:{','.join(choices)}}}}}"
    }):
        # Get the result
...
def test_text_placeholder():
    ...
    with patch.object(manager, '_read_prompts', return_value={
        "TEXT_TEST": "{{text:Hello {name}!,name=World}}"
    }):
...
def test_missing_variables_in_text():
    ...
    with patch.object(manager, '_read_prompts', return_value={
        "MISSING_VAR": "{{text:Hello {name}!}}"  # No 'name' defined
    }):
...
def test_invalid_placeholder():
    ...
    with patch.object(manager, '_read_prompts', return_value={
        "INVALID": "This is an {{invalid}} placeholder"
    }):
